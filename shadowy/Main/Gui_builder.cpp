...
void Gui_builder::save_building(std::string& file)
{
    remove(file.c_str());
...
void Gui_builder::show_propriets_geral()
{
    ...
    if (ImGui::BeginCombo("##itens", current_item.c_str()))
    {
        for (size_t n = 0; n < forms.size(); n++)
        {
            ...
            std::string item = forms[n].name_form + ":" + std::to_string(forms[n].form_id);
        ...
        {
            std::string item = buttons[n].name_item + ":" + std::to_string(buttons[n].btn_id);
            const bool is_selected = (current_item == item);
            if (ImGui::Selectable(item.c_str(), is_selected))
            {
        ...
        {
            std::string item = texts[n].name_text + ":" + std::to_string(texts[n].text_id);
            const bool is_selected = (current_item == item);
            if (ImGui::Selectable(item.c_str(), is_selected))
            {
        ...
        {
            std::string item = checkbox[n].name + ":" + std::to_string(checkbox[n].id);
            const bool is_selected = (current_item == item);
            if (ImGui::Selectable(item.c_str(), is_selected))
            {
        ...
        {
            std::string item = toggle[n].name + ":" + std::to_string(toggle[n].id);
            const bool is_selected = (current_item == item);
            if (ImGui::Selectable(item.c_str(), is_selected))
            {
        ...
        {
            std::string item = SliderI[n].name + ":" + std::to_string(SliderI[n].slider_id);
            const bool is_selected = (current_item == item);
            if (ImGui::Selectable(item.c_str(), is_selected))
            {
        ...
        {
            std::string item = SliderF[n].name + ":" + std::to_string(SliderF[n].slider_id);
            const bool is_selected = (current_item == item);
            if (ImGui::Selectable(item.c_str(), is_selected))
            {
        ...
        {
            std::string item = Radio[n].name + ":" + std::to_string(Radio[n].id);
            const bool is_selected = (current_item == item);
            if (ImGui::Selectable(item.c_str(), is_selected))
            {
        ...
        {
            std::string item = childs[n].a.name + ":" + std::to_string(childs[n].a.id);
            const bool is_selected = (current_item == item);
            if (ImGui::Selectable(item.c_str(), is_selected))
            {
        ...
        {
            std::string item = label[n].name + ":" + std::to_string(label[n].id);
            const bool is_selected = (current_item == item);
            if (ImGui::Selectable(item.c_str(), is_selected))
            {
...
void Gui_builder::show_propriedades_basic(indentification_basic& obj_basic)
{
    ImGui::SetNextWindowPos({ 0,100 });
    ImGui::SetNextWindowSize({ 200,700 - 100 });
    ImGui::Begin("PROPRIEDADES", nullptr, ImGuiWindowFlags_NoBringToFrontOnFocus);
...
void Gui_builder::show_propriedades_slider(identification_slider& slider)
{
    ImGui::SetNextWindowPos({ 0,100 });
    ImGui::SetNextWindowSize({ 200,700 - 100 });
    ImGui::Begin("PROPRIEDADES", nullptr, ImGuiWindowFlags_NoBringToFrontOnFocus);
